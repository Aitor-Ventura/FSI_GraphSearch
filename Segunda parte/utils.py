...
class OrderedQueue(Queue):
    """A queue that pops its elements in order of least cost using heuristics."""

    def __init__(self, f=lambda x: 0):
        self.A = []
        self.f = f

    def append(self, item):
        bisect.insort(self.A, (item.path_cost + self.f(item), item))
